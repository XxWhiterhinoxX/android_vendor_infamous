...
usage()
{
    ...
    echo -e "  build-infamous.sh [options] device"
    echo -e ""
    echo -e ${txtbld}"  Options:"${txtrst}
    echo -e "    -c# Cleanin options before build:"
    echo -e "        1 - make infamous"
    echo -e "        2 - make clean"
    echo -e "        3 - make dirty"
    echo -e "        4 - make magic"
    ...
    echo -e "    ./build-infamous.sh -p -o3 -j18 hammerhead"
    echo -e ""
    exit 1
}

# colors
. ./vendor/infamous/tools/colors

if [ ! -d ".repo" ]; then
    echo -e ${red}"No .repo directory found.  Is this an Android build tree?"${txtrst}
    exit 1
fi
if [ ! -d "vendor/infamous" ]; then
    echo -e ${red}"No vendor/infamous directory found.  Is this an infamous build tree?"${txtrst}
    exit 1
fi
...
# get current version
eval $(grep "^INFAMOUS_VERSION_" vendor/infamous/config/common.mk | sed 's/ *//g')
VERSION="$INFAMOUS_VERSION_MAJOR.$INFAMOUS_VERSION_MINOR.$INFAMOUS_VERSION_MAINTENANCE"

echo -e ${cya}"Building ${ppl}Infamous ${bldylw}$VERSION"${txtrst}

if [ "$opt_clean" -eq 1 ]; then
    echo -e ""
    echo -e ${bldblu}"Building INFAMOUS"${txtrst}
    echo -e ""
...
lunch "infamous_$device-userdebug";
...
if [ "$opt_log" -ne 0 ]; then
    ...
    make -j"$opt_jobs" infamous 2>&1 | tee build-logs/infamous_$device-$(date "+%Y%m%d").txt
else
    # build normally
    make -j"$opt_jobs" infamous
fi

# cleanup unused built
rm -f $OUTDIR/target/product/$device/infamous_*-ota*.zip
